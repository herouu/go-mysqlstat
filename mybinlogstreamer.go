...
func (s *BinlogStreamer) GetEvent(ctx context.Context) (*replication.BinlogEvent, error) {
	if s.err != nil {
...
// return nil event
func (s *BinlogStreamer) GetEventWithStartTime(ctx context.Context, startTime time.Time) (*replication.BinlogEvent, error) {
	if s.err != nil {
		return nil, ErrNeedSyncAgain
	}
...
func (s *BinlogStreamer) DumpEvents() []*replication.BinlogEvent {
	count := len(s.ch)
	events := make([]*replication.BinlogEvent, count)
	for i := range events {
		events[i] = <-s.ch
	}
	return events
...
func NewBinlogStreamerWithChanSize(chanSize int) *BinlogStreamer {
	...
	s.ch = make(chan *replication.BinlogEvent, chanSize)
...
func (s *BinlogStreamer) AddEventToStreamer(ev *replication.BinlogEvent) error {
	select {
	case s.ch <- ev:
		return nil
	case err := <-s.ech:
